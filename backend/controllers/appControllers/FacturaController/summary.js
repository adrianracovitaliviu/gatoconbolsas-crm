...
const summary = async (req, res) => {
  try {
    ...
    const totalFacturas = response[0].totalFactura ? response[0].totalFactura[0] : 0;
    ...
    const statusResultMap = statusResult.map((item) => {
      return {
        ...item,
        percentage: Math.round((item.count / totalFacturas.count) * 100),
      };
    });
    ...
    const finalResult = {
      total: totalFacturas?.total.toFixed(2),
    ...
    return res.status(200).json({
      success: true,
      result: finalResult,
      message: `Successfully found all Facturas for the last ${defaultType}`,
    });
  } catch (error) {
    console.log(error);
