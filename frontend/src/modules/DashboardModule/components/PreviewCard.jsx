...
const defaultAlbaranStatistics = [
  {
    tag: 'borrador',
    value: 0,
  },
...
const defaultFacturaStatistics = [
  {
    tag: 'borrador',
    value: 0,
  },
...
export default function PreviewCard({
  title = 'Resumen',
  statistics = defaultAlbaranStatistics,
  isLoading = false,
  entity = 'Factura',
}) {
  const statisticsMap = useMemo(() => {
    if (entity === 'Factura') {
      return defaultFacturaStatistics.map((defaultStat) => {
      ...
      return defaultAlbaranStatistics.map((defaultStat) => {
        const matchedStat = Array.isArray(statistics)
          ? statistics.find((stat) => stat.tag === defaultStat.tag)
          : null;
        return matchedStat || defaultStat;
