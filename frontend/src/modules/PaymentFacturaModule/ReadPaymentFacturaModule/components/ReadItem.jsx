...
export default function ReadItem({ config, selectedItem }) {
  ...
  useEffect(() => {
    ...
    if (currentResult) {
      const { Factura, _id, ...others } = currentResult;
      setCurrentErp({ ...others, ...Factura, _id });
    }
  ...
  return (
    <>
      ...
      <Descriptions title={`Cliente : ${currentErp.client.company}`}>
        <Descriptions.Item label="Dirección">{currentErp.client.address}</Descriptions.Item>
      ...
      <Row>
        <Col sm={24} md={12}>
          <Typography.Title level={5}>Información de pago :</Typography.Title>
        </Col>
        <Col sm={24} md={12} style={{ textAlign: 'right' }}>
          <Button icon={<ExportOutlined />}>Ver factura</Button>
        </Col>
      </Row>
      <div
        style={{
